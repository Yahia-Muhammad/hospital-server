...
const addDoctor = asyncWrapper(async (req, res, next) => {
  ...
  const newDoctor = new Anesthesiology({
    name,
    email,
    phone,
    specialization,
...
const editDoctor = asyncWrapper(async (req, res) => {
  const doctor = await Anesthesiology.updateOne(
    { _id: req.params.id },
    { $set: { ...req.body } }
  );
...
const deleteDoctor = asyncWrapper(async (req, res) => {
  await Anesthesiology.deleteOne({ _id: req.params.id });
