...
const editDoctor = asyncWrapper(async (req, res) => {
  const doctor = await Surgery.updateOne(
    { _id: req.params.id },
    { $set: { ...req.body } }
  );
...
const deleteDoctor = asyncWrapper(async (req, res) => {
  await Surgery.deleteOne({ _id: req.params.id });
