...
const editDoctor = asyncWrapper(async (req, res) => {
  const doctor = await Heart.updateOne(
    { _id: req.params.id },
    { $set: { ...req.body } }
  );
...
const deleteDoctor = asyncWrapper(async (req, res) => {
  await Heart.deleteOne({ _id: req.params.id });
