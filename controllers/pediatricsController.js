...
const addDoctor = asyncWrapper(async (req, res, next) => {
  ...
  const newDoctor = new Pediatrics({
    name,
    email,
    phone,
    specialization,
...
const editDoctor = asyncWrapper(async (req, res) => {
  const doctor = await Pediatrics.updateOne(
    { _id: req.params.id },
    { $set: { ...req.body } }
  );
...
const deleteDoctor = asyncWrapper(async (req, res) => {
  await Pediatrics.deleteOne({ _id: req.params.id });
