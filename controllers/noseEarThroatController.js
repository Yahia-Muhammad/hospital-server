...
const addDoctor = asyncWrapper(async (req, res, next) => {
  ...
  const newDoctor = new NoseEarThroat({
    name,
    email,
    phone,
    specialization,
...
const editDoctor = asyncWrapper(async (req, res) => {
  const doctor = await NoseEarThroat.updateOne(
    { _id: req.params.id },
    { $set: { ...req.body } }
  );
...
const deleteDoctor = asyncWrapper(async (req, res) => {
  await NoseEarThroat.deleteOne({ _id: req.params.id });
