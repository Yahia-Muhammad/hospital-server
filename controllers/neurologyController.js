...
const editDoctor = asyncWrapper(async (req, res) => {
  const doctor = await Neurology.updateOne(
    { _id: req.params.id },
    { $set: { ...req.body } }
  );
...
const deleteDoctor = asyncWrapper(async (req, res) => {
  await Neurology.deleteOne({ _id: req.params.id });
