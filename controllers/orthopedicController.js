...
const addDoctor = asyncWrapper(async (req, res, next) => {
  ...
  const newDoctor = new Orthopedic({
    name,
    email,
    phone,
    specialization,
...
const editDoctor = asyncWrapper(async (req, res) => {
  const doctor = await Orthopedic.updateOne(
    { _id: req.params.id },
    { $set: { ...req.body } }
  );
...
const deleteDoctor = asyncWrapper(async (req, res) => {
  await Orthopedic.deleteOne({ _id: req.params.id });
