...
const getDoctors = asyncWrapper(async (req, res) => {
  const doctors = await DermatologyCosmetology.find({}, { __v: false });
  res.status(200).json({ status: httpStatusText.SUCCESS, data: { doctors } });
});

const getDoctor = asyncWrapper(async (req, res, next) => {
  const doctor = await DermatologyCosmetology.findById(req.params.id);
  if (!doctor) {
...
const addDoctor = asyncWrapper(async (req, res, next) => {
  ...
  const newDoctor = new DermatologyCosmetology({
    name,
    email,
    phone,
    specialization,
...
const editDoctor = asyncWrapper(async (req, res) => {
  const doctor = await DermatologyCosmetology.updateOne(
    { _id: req.params.id },
    { $set: { ...req.body } }
  );
...
const deleteDoctor = asyncWrapper(async (req, res) => {
  await DermatologyCosmetology.deleteOne({ _id: req.params.id });
